...
class trajGenerator:
    ...
    def get_des_state(self,t):
        ...
        pos  = coeff@polyder(t,order = self.order)
        vel  = coeff@polyder(t,1,order = self.order)
        accl = coeff@polyder(t,2,order = self.order)
        jerk = coeff@polyder(t,3,order = self.order)
